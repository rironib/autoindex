			...
			if ((($_FILES['f']['size'][$i] / 1024) / 1024) < $maximum) {
				if ($ext->extension == 'jad') {
					...
					if ($url) {
						...
						if ($snoopy->fetch($url)) {
							...
							if (file_put_contents($path . "/" . $_name, $snoopy->results)) {
								$message .= "<div class='alert alert-success'>" . $_name . $lang->file_uploaded . "</div>";
					...
					if (move_uploaded_file($_FILES['f']['tmp_name'][$i], $path . "/" . $_FILES['f']['name'][$i])) {
						$message .= "<div class='alert alert-success'>" . $_FILES['f']['name'][$i] . $lang->file_uploaded . "</div>";
						$dirid = $db->get_row("SELECT id FROM `" . MAI_PREFIX . "files` WHERE `path`='" . substr($path, 2) . "'")->id;
						...
						$message .= "<div class='alert alert-danger'>" . $_FILES['f']['name'][$i] . $lang->file_not_uploaded . "</div>";
				}
			} else {
				@unlink($_FILES['f']['name'][$i]);
				$message .= "<div class='alert alert-danger'>" . $_FILES['f']['name'][$i] . $lang->file_too_big . "</div>";
			}
		}
		++$i;
	}
